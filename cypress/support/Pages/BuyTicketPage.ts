class BuyTicketPage {
  ...
  enterTravelDetails(fromCity: string, toCity: string, departOn: string, note: string) {
    const[year, month, day] = [departOn.split("/")[0], departOn.split("/")[1],  parseInt(departOn.split("/")[2], 10)]
    ...
    cy.get('.ui-state-default').eq(day - 1).click()
    cy.get('#notes').type(note)
    cy.get('#deliverymethod_1').check()
  }


  enterBillingDetails(billingName: string, phoneNumber: string, email: string, country: string, address1: string, city: string, county: string, postcode:string) {
