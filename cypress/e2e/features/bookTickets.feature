...
Scenario: Buying a ticket on the site, then verify card reject error message
  ...
  And User click buy ticket link on the navigation bar and confirms "dummy return ticket" radio button is checked
  ...
  Then a message "An error occurred while processing the card. (The card has been declined. Please, contact your card issuer for more information.)" is displayed 